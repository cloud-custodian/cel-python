...
def test_age_to_duration():
    ...
    assert C7N_Rewriter.age_to_duration(0.5) == '"12h"'
    assert C7N_Rewriter.age_to_duration(0.084) == '"2h57s"'
    assert C7N_Rewriter.age_to_duration(0.011) == '"15m50s"'


@fixture
def mock_logical_connector(monkeypatch):
    logical_connector = Mock(return_value=sentinel.rewritten)
...
def test_c7n_rewrite(mock_logical_connector):
    assert (
        C7N_Rewriter.c7n_rewrite('name: policy\nfilters: "text"\n')
        == sentinel.rewritten
    )
...
def mock_type_value_rewrite(monkeypatch):
    type_value_rewrite = Mock(return_value=str(sentinel.rewritten))
...
def mock_type_marked_for_op_rewrite(monkeypatch):
    type_marked_for_op_rewrite = Mock(return_value=str(sentinel.rewritten))
    monkeypatch.setattr(
        C7N_Rewriter, "type_marked_for_op_rewrite", type_marked_for_op_rewrite
    )
...
def mock_type_image_age_rewrite(monkeypatch):
    type_image_age_rewrite = Mock(return_value=str(sentinel.rewritten))
...
def mock_type_event_rewrite(monkeypatch):
    type_event_rewrite = Mock(return_value=str(sentinel.rewritten))
...
def test_logical_connector_and(mock_type_value_rewrite):
    assert C7N_Rewriter.logical_connector(
        sentinel.resource, {"and": [{"type": "value"}]}
    ) == str(sentinel.rewritten)
    assert mock_type_value_rewrite.mock_calls == [
        call(sentinel.resource, {"type": "value"})
    ]
...
def test_logical_connector_or(mock_type_value_rewrite):
    ...
    assert C7N_Rewriter.logical_connector(
        sentinel.resource, {"or": [{"type": "value"}]}
    ) == str(sentinel.rewritten)
    assert mock_type_value_rewrite.mock_calls == [
        call(sentinel.resource, {"type": "value"})
    ]
...
def test_logical_connector_not_1(mock_type_value_rewrite):
    ...
    assert (
        C7N_Rewriter.logical_connector(sentinel.resource, not_1)
        == f"! ({str(sentinel.rewritten)})"
    )
    assert mock_type_value_rewrite.mock_calls == [
        call(sentinel.resource, {"type": "value"})
    ]
...
def test_logical_connector_not_2(mock_type_value_rewrite):
    ...
    assert C7N_Rewriter.logical_connector(sentinel.resource, not_2) == expected_2
    assert mock_type_value_rewrite.mock_calls == [
...
def mock_key_to_cel(monkeypatch):
    key_to_cel = Mock(return_value=str(sentinel.rewritten))
...
def mock_value_to_cel(monkeypatch):
    value_to_cel = Mock(return_value=str(sentinel.rewritten))
...
def mock_value_from_to_cel(monkeypatch):
    value_from_to_cel = Mock(return_value=str(sentinel.rewritten))
...
def test_type_value_rewrite(mock_key_to_cel, mock_value_to_cel):
    ...
    assert C7N_Rewriter.type_value_rewrite(sentinel.resource, clause) == str(
        sentinel.rewritten
    )
...
def test_type_value_rewrite_present(mock_key_to_cel, mock_value_to_cel):
    ...
    assert mock_key_to_cel.mock_calls == [call("key")]
    assert mock_value_to_cel.mock_calls == [
        call(str(sentinel.rewritten), "__present__", None)
    ]
...
def test_primitive_value(mock_type_value_rewrite):
    assert C7N_Rewriter.primitive(sentinel.resource, {"type": "value"}) == str(
        sentinel.rewritten
    )
    assert mock_type_value_rewrite.mock_calls == [
        call(sentinel.resource, {"type": "value"})
...
def test_value_to_cel_non_bool():
    ...
    assert C7N_Rewriter.value_to_cel("key", "gt", 42) == "key > 42"
    assert (
        C7N_Rewriter.value_to_cel("key", "gt", 42, value_type="age")
        == 'now - duration("42d") > timestamp(key)'
    )
    assert (
        C7N_Rewriter.value_to_cel("key", "gt", 42, value_type="integer")
        == "int(key) > 42"
    )
    assert (
    ...
    assert (
        C7N_Rewriter.value_to_cel("key", "gt", 42, value_type="size")
        == "size(key) > 42"
    )
    assert (
...
def test_key_to_cel():
    ...
    assert C7N_Rewriter.key_to_cel("Key.Subkey") == 'resource["Key"]["Subkey"]'
    assert (
...
def test_marked_for_op_rewrite(mock_key_to_cel):
    ...
    assert (
        C7N_Rewriter.type_marked_for_op_rewrite(sentinel.resource, clause) == expected
    )


def test_primitive_mark_for_op(mock_type_marked_for_op_rewrite):
    assert C7N_Rewriter.primitive(sentinel.resource, {"type": "marked-for-op"}) == str(
        sentinel.rewritten
    )
...
def test_primitive_image_age(mock_type_image_age_rewrite):
    assert C7N_Rewriter.primitive(sentinel.resource, {"type": "image-age"}) == str(
        sentinel.rewritten
    )
...
def test_event_rewrite():
    ...
    expected = 'event.detail.responseElements.functionName.matches("^(custodian-.*)")'
    assert C7N_Rewriter.type_event_rewrite(sentinel.resource, clause) == expected


def test_primitive_event(mock_type_event_rewrite):
    assert C7N_Rewriter.primitive(sentinel.resource, {"type": "event"}) == str(
        sentinel.rewritten
    )
...
def test_age_rewrite():
    ...
    expected = 'now - timestamp(resource.StartTime) > duration("21d")'
...
def test_flow_logs_rewrite():
    ...
    clause_3 = {
        ...
        "log-format": "this",
        "destination": "that",
        "deliver-status": "the-other-thing",
    }
...
def test_type_vpc_rewrite():
    clause_0 = {
        ...
        "value_from": {
            ...
            "format": "json",
            "expr": 'not_null(offhours_exceptions."{account_id}".account, "[]")'.format(
...
def test_onhour_rewrite():
    clause_0 = {"default_tz": "et", "onhour": 7, "opt-out": True, "type": "onhour"}
...
def test_used_rewrite():
    clause_0 = {"type": "used"}
    expected_0 = (
        'resource["LaunchConfigurationName"] in all_launch_configuration_names()'
    )
