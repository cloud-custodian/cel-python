...
def test_parse_cidr():
    ...
    assert celpy.c7nlib.parse_cidr("192.168.100.0") in celpy.c7nlib.parse_cidr(
        "192.168.100.0/22"
    )
    assert celpy.c7nlib.parse_cidr("192.168.100.0").packed == bytes([192, 168, 100, 0])
    assert celpy.c7nlib.parse_cidr("localhost") is None
    assert not celpy.c7nlib.parse_cidr("localhost") in celpy.c7nlib.parse_cidr(
        "192.168.100.0/22"
    )
    assert celpy.c7nlib.parse_cidr("192.168.100.0/22") in celpy.c7nlib.parse_cidr(
        "192.168.100.0/22"
    )
...
value_from_examples = [
    (
        ...
        "text",
        b"data\n",
        celpy.celtypes.ListType([celpy.celtypes.StringType("data")]),
    ),
    (
        ".txt",
        "gzip",
    ...
    (
        ...
        celpy.celtypes.ListType(
            [
                celpy.celtypes.MapType(
                    {
                        celpy.celtypes.StringType("row"): celpy.celtypes.StringType(
                            "1"
                        ),
                        celpy.celtypes.StringType("value"): celpy.celtypes.StringType(
                            "42"
                        ),
                    }
                ),
...
def mock_urllib_request(monkeypatch, request):
    ...
    mock_os = Mock(splitext=Mock(return_value=("path", suffix)))
...
def test_marked_key_good():
    tags_good = celpy.celtypes.ListType(
        [
            celpy.celtypes.MapType(
                {
                    celpy.celtypes.StringType("Key"): celpy.celtypes.StringType(
                        "c7n-tag-compliance"
                    ),
                    celpy.celtypes.StringType("Value"): celpy.celtypes.StringType(
                        "hello:stop@2020-09-10"
                    ),
                }
            ),
        ]
    ...
    doc = celpy.c7nlib.marked_key(
        tags_good, celpy.celtypes.StringType("c7n-tag-compliance")
    )
    assert doc.get(celpy.celtypes.StringType("message")) == celpy.celtypes.StringType(
        "hello"
    )
    assert doc.get(celpy.celtypes.StringType("action")) == celpy.celtypes.StringType(
        "stop"
    )
    assert doc.get(
        celpy.celtypes.StringType("action_date")
    ) == celpy.celtypes.TimestampType("2020-09-10")
...
@pytest.fixture
def mock_manager():
    datapoints = [{"Average": str(sentinel.average)}]
    health_events = [
        {
            "category": "issue",
    ...
    cloudwatch_client = Mock(
        ...
        get_metric_statistics=Mock(return_value={"Datapoints": datapoints}),
    )
    ec2_client = Mock(
        name="ec2_client",
    ...
    elb_client = Mock(
        ...
        describe_load_balancer_attributes=Mock(
            return_value={"LoadBalancerAttributes": [{"Enabled": True}]}
        ),
    )
    elbv2_client = Mock(
        name="elbv2_client",
    ...
    health_client = Mock(
        name="health_client",
        describe_events=Mock(return_value={"events": health_events}),
    )
    kms_client = Mock(
        name="kms_client",
    ...
    shield_client = Mock(
        name="shield_client",
        describe_subscription=Mock(return_value={"Subscription": str(sentinel.shield)}),
    )
    clients = {
        "ec2": ec2_client,
    ...
    manager = Mock(
        ...
        get_model=Mock(
            return_value=Mock(dimension="InstanceId", id="InstanceId", service="ec2")
        ),
...
def mock_filter_class():
    ...
    mock_parser = Mock(
        ...
        parse=Mock(
            return_value={
                ...
                "on": [{"days": [1, 2, 3, 4, 5], "hour": 6}, {"days": [0], "hour": 10}],
                "tz": "pt",
            }
        ),
    )
    ...
    class InstanceImageMixin:
        get_instance_image = Mock(
            return_value={
                "CreationDate": "2020-09-10T11:12:13Z",
...
def test_image_age_good(celfilter_instance):
    ...
    with celpy.c7nlib.C7NContext(filter=mock_filter):
        doc = celpy.c7nlib.image(resource)
    assert doc.get(
        celpy.celtypes.StringType("CreationDate")
...
def test_image_age_missing(celfilter_instance, mock_manager):
    ...
    assert doc.get(
        celpy.celtypes.StringType("CreationDate")
    ) == celpy.celtypes.TimestampType("2000-01-01T01:01:01.000Z")
...
def test_get_raw_metrics(celfilter_instance, mock_manager):
    ...
    request = celpy.celtypes.MapType(
        {
            celpy.celtypes.StringType("Namespace"): celpy.celtypes.StringType(
                "AWS/EC2"
            ),
            celpy.celtypes.StringType("MetricName"): celpy.celtypes.StringType(
                "CPUUtilization"
            ),
            celpy.celtypes.StringType("Dimensions"): celpy.celtypes.MapType(
                {
                    celpy.celtypes.StringType("Name"): celpy.celtypes.StringType(
                        "InstanceId"
                    ),
                    celpy.celtypes.StringType("Value"): celpy.celtypes.StringType(
                        "i-1234567890abcdef0"
            ...
            celpy.celtypes.StringType("StartTime"): now
            - celpy.celtypes.DurationType("4d"),
...
def test_get_metrics(celfilter_instance, mock_manager):
    ...
    request = celpy.celtypes.MapType(
        {
            ...
            celpy.celtypes.StringType("Statistic"): celpy.celtypes.StringType(
                "Average"
            ),
            celpy.celtypes.StringType("StartTime"): now
            - celpy.celtypes.DurationType("4d"),
    ...
    with celpy.c7nlib.C7NContext(filter=mock_filter):
        doc = celpy.c7nlib.get_metrics(resource, request)
...
def test_get_related_ids(celfilter_instance):
    ...
    resource = celpy.celtypes.MapType({})
    with celpy.c7nlib.C7NContext(filter=mock_filter):
        doc = celpy.c7nlib.get_related_ids(resource)
    assert doc == celpy.celtypes.ListType(
        [
...
def test_security_group(celfilter_instance):
    ...
    with celpy.c7nlib.C7NContext(filter=mock_filter):
        doc = celpy.c7nlib.security_group(resource)
    assert doc == celpy.celtypes.ListType(
        [
            celpy.celtypes.MapType(
                {
...
def test_subnet(celfilter_instance):
    ...
    resource = celpy.celtypes.MapType({})
    with celpy.c7nlib.C7NContext(filter=mock_filter):
        doc = celpy.c7nlib.subnet(resource)
    assert doc.get(celpy.celtypes.StringType("SubnetID")) == celpy.celtypes.StringType(
        "subnet-12345678"
    )
...
def test_flow_logs(celfilter_instance):
    ...
    resource_1 = celpy.celtypes.MapType(
        {
            celpy.celtypes.StringType("InstanceId"): celpy.celtypes.StringType(
                "i-123456789"
            )
        }
    )
    ...
    with celpy.c7nlib.C7NContext(filter=mock_filter):
        doc_2 = celpy.c7nlib.flow_logs(resource_2)
    assert doc_2 == []
...
def test_vpc(celfilter_instance):
    ...
    with celpy.c7nlib.C7NContext(filter=mock_filter):
        doc_1 = celpy.c7nlib.vpc(resource_1)
    assert doc_1 == vpc_1
...
def test_credentials(celfilter_instance):
    ...
    resource_1 = celpy.celtypes.MapType(
        {
            celpy.celtypes.StringType("ResourceType"): celpy.celtypes.StringType(
                "iam-user"
            )
        }
    )
    with celpy.c7nlib.C7NContext(filter=mock_filter):
        doc_1 = celpy.c7nlib.credentials(resource_1)
...
def test_kms_alias(celfilter_instance):
    ...
    with celpy.c7nlib.C7NContext(filter=mock_filter):
        doc_1 = celpy.c7nlib.kms_alias(resource_1)
...
def test_kms_key(celfilter_instance):
    ...
    with celpy.c7nlib.C7NContext(filter=mock_filter):
        doc_1 = celpy.c7nlib.kms_key(resource_1)
...
def test_describe_subscription_filters(celfilter_instance):
    ...
    log_group_doc = {
        ...
        "logGroupName": str(sentinel.log_group_name),
    }
    with celpy.c7nlib.C7NContext(filter=mock_filter):
...
def test_describe_db_snapshot_attributes(celfilter_instance):
    ...
    rds_snapshot_doc = {
        ...
        "SnapshotId": str(sentinel.snapshot_id),
    }
    with celpy.c7nlib.C7NContext(filter=mock_filter):
...
def test_C7N_interpreted_runner(celfilter_instance):
    ...
    cel_env = celpy.Environment(
        annotations=decls, runner_class=celpy.c7nlib.C7N_Interpreted_Runner
    )
...
def test_C7N_CELFilter_image(celfilter_instance):
    ...
    assert image == {
        "CreationDate": celpy.celtypes.TimestampType("2020-09-10T11:12:13Z"),
        "Name": str(sentinel.name),
    }
...
def test_C7N_CELFilter_get_raw_metrics(celfilter_instance, mock_manager):
    ...
    assert cloudwatch_client.mock_calls == [
        call.get_metric_statistics(**expected_request)
    ]
...
def test_C7N_CELFilter_flow_logs(celfilter_instance, mock_manager):
    ...
    assert flow_logs == celpy.celtypes.ListType(
        [
            celpy.celtypes.MapType(
                {
                    celpy.celtypes.StringType("ResourceId"): celpy.celtypes.StringType(
                        "i-123456789"
                    )
                }
            )
...
def test_C7N_CELFilter_credentials(celfilter_instance):
    ...
    assert mock_filter.get_credential_report.mock_calls == [
        call({"ResourceType": "ec2", "InstanceId": "i-123456789"})
    ]
...
def test_C7N_CELFilter_get_load_balancer(celfilter_instance, mock_manager):
    mock_filter = celfilter_instance["the_filter"]
    elb_doc = {
        "ResourceType": "app-elb",
        "LoadBalancerArn": "arn:us-east-1:app-elb:123456789:etc",
    }
    with celpy.c7nlib.C7NContext(filter=mock_filter):
    ...
    assert mock_manager[
        "elbv2_client"
    ].describe_load_balancer_attributes.mock_calls == [
...
def test_C7N_CELFilter_get_raw_health_events(celfilter_instance, mock_manager):
    ...
    request = celpy.celtypes.MapType(
        {
            celpy.celtypes.StringType("services"): celpy.celtypes.ListType(
                [celpy.celtypes.StringType("ELASTICFILESYSTEM")]
            ),
            celpy.celtypes.StringType("regions"): celpy.celtypes.ListType(
                [
                    celpy.celtypes.StringType("us-east-1"),
                    celpy.celtypes.StringType("global"),
                ]
            ),
            celpy.celtypes.StringType("eventStatusCodes"): celpy.celtypes.ListType(
                [
...
def test_C7N_CELFilter_get_health_events(celfilter_instance, mock_manager):
    ...
    directory_doc = {
        ...
        "arn": "arn:us-east-1:app-elb:123456789:etc",
    }
    with celpy.c7nlib.C7NContext(filter=mock_filter):
...
def test_C7N_CELFilter_shield_subscription(celfilter_instance):
    ...
    assert mock_filter.account_shield_subscriptions.mock_calls == [call(account_doc)]
...
def test_C7N_CELFilter_web_acls(celfilter_instance, mock_manager):
    ...
    assert mock_filter.manager.get_resource_manager.mock_calls == [call("waf")]
    assert mock_manager["waf_resource_manager"].resources.mock_calls == [call()]
